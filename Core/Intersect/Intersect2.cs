...
{
    public static class Intersect2
    {
        ...
        public static void MultipleElementsToMultipleCategory(ExternalCommandData commandData)
        {
            ...
            foreach (Element e in Get_allElements(commandData))
            {
                ElementId eID = e.Id;
                ...
                foreach (BuiltInCategory bic in GetLists.BuiltCategories(false))
                {

                    ElementClassFilter DUFilter4 = null;
                    if (bic == BuiltInCategory.OST_CableTray)
                    {
                        DUFilter4 = new ElementClassFilter(typeof(CableTray));
                    }
                    if (bic == BuiltInCategory.OST_Conduit)
                    {
                        DUFilter4 = new ElementClassFilter(typeof(Conduit));
                    }
                    if (bic == BuiltInCategory.OST_DuctCurves)
                    {
                        DUFilter4 = new ElementClassFilter(typeof(Duct));
                    }
                    if (bic == BuiltInCategory.OST_PipeCurves)
                    {
                        DUFilter4 = new ElementClassFilter(typeof(Pipe));
                    }
                    if (bic == BuiltInCategory.OST_FlexDuctCurves)
                    {
                        DUFilter4 = new ElementClassFilter(typeof(FlexDuct));
                    }
                    if (bic == BuiltInCategory.OST_FlexPipeCurves)
                    {
                        DUFilter4 = new ElementClassFilter(typeof(FlexPipe));
                    }

                    ElementCategoryFilter DU2Categoryfilter4 = new ElementCategoryFilter(bic);
        ...
        public static void MultipleElementsToMultipleFamilyInstances(ExternalCommandData commandData)
        {
            ...
            foreach (Element e in Get_allElements(commandData))
            {
                ...
                IList<BuiltInCategory> bics_fi = GetLists.BuiltCategories(true);
        ...
        public static void MultipleFamilyInstanceToMultipleFamilyInstances_BBox(ExternalCommandData commandData) // Family Instance vs Family Instance
        {
            ...
            foreach (BuiltInCategory bic in GetLists.BuiltCategories(true))
            {
        ...
        public static void MultipleElementsToLinksFamilyInstance(ExternalCommandData commandData, List<Element> lista_links)
        {
            ...
            foreach (Element e in allElements)
            {
                ...
                foreach (RevitLinkInstance link in lista_links)
                {
                    ...
                    if (solid != null)
                    {
                        _ = collector4.WherePasses(new ElementIntersectsSolidFilter(solid));
                    }
                    _ = collector4.WherePasses(filter4).ToElements();
